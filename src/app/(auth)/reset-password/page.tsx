...
function ResetPasswordForm() {
  ...
  if (!token) {
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
        ...
        <p className="text-sm text-slate-400 mb-4">{t.invalidLinkDesc}</p>
  ...
  if (success) {
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
        ...
        <h2 className="text-xl font-bold text-white mb-2">{t.resetSuccessTitle}</h2>
        <p className="text-sm text-slate-400">{t.resetSuccessDesc}</p>
      </motion.div>
    );
  }
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
      ...
      <h1 className="text-xl font-bold text-white mb-2">{t.resetTitle}</h1>
      <p className="text-sm text-slate-400 mb-6">{t.resetDesc}</p>
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input id="reset-new-password" type={showPassword ? 'text' : 'password'} value={newPassword} onChange={e => setNewPassword(e.target.value)}
              required placeholder={t.newPasswordPlaceholder} minLength={6}
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input id="reset-confirm-password" type={showPassword ? 'text' : 'password'} value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)}
              required placeholder={t.confirmPasswordLabel}
