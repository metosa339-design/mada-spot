...
export default function ProviderDetail() {
  ...
  if (!provider) {
    return (
      <div className="min-h-screen bg-[#F8FAFC] flex flex-col items-center justify-center gap-4" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
        <Users className="w-16 h-16 text-[#CBD5E1]" />
        <h1 className="text-xl font-semibold text-[#0F172A]">Prestataire non trouvé</h1>
        <Link
          href="/prestataires"
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 md:py-12 pb-16">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            {/* Header */}
            <div className="bg-white rounded-xl p-6 md:p-8 border border-[#E2E8F0]">
              <div className="flex items-start justify-between mb-4 gap-4">
                <div>
                  ...
                  <h1 className="text-[28px] sm:text-[36px] font-semibold tracking-[-0.03em] text-[#0F172A]">{name}</h1>
                  <div className="flex items-center gap-2 mt-2 text-[#64748B] text-[13px]">
                    <MapPin className="w-4 h-4" />
                    <span>{provider.district ? `${provider.district}, ` : ''}{provider.city}{provider.region ? `, ${provider.region}` : ''}</span>
                  </div>
                </div>
                <button onClick={handleShare} className="p-2.5 bg-white border border-[#E2E8F0] hover:border-[#CBD5E1] rounded-lg text-[#64748B] hover:text-[#0F172A] transition-colors shrink-0">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {provider.languages?.length > 0 && (
                  <div className="flex items-start gap-3 p-4 bg-white rounded-lg border border-[#E2E8F0]">
                    ...
                    <div>
                      <p className="text-[11px] uppercase tracking-[0.15em] text-[#64748B]">Langues parlées</p>
                      <p className="text-[#0F172A] font-medium text-[13px] mt-0.5">{provider.languages.join(', ')}</p>
                    </div>
                  </div>
                )}
                {provider.experience && (
                  <div className="flex items-start gap-3 p-4 bg-white rounded-lg border border-[#E2E8F0]">
                    ...
                    <div>
                      <p className="text-[11px] uppercase tracking-[0.15em] text-[#64748B]">Expérience</p>
                      <p className="text-[#0F172A] font-medium text-[13px] mt-0.5">{provider.experience}</p>
                    </div>
                  </div>
                )}
                {(() => {
                ...
                {provider.vehicleType && (
                  <div className="flex items-start gap-3 p-4 bg-white rounded-lg border border-[#E2E8F0]">
                    ...
                    <div>
                      <p className="text-[11px] uppercase tracking-[0.15em] text-[#64748B]">Véhicule</p>
                      <p className="text-[#0F172A] font-medium text-[13px] mt-0.5">
                        {provider.vehicleType}
                ...
                {provider.licenseNumber && (
                  <div className="flex items-start gap-3 p-4 bg-white rounded-lg border border-[#E2E8F0]">
                    ...
                    <div>
                      <p className="text-[11px] uppercase tracking-[0.15em] text-[#64748B]">Licence / Agrément</p>
                      <p className="text-[#0F172A] font-medium text-[13px] font-mono mt-0.5">{provider.licenseNumber}</p>
                    </div>
                  </div>
                )}
                {provider.certifications?.length > 0 && (
                  <div className="flex items-start gap-3 p-4 bg-white rounded-lg border border-[#E2E8F0]">
                    <Award className="w-4 h-4 text-[#FF6B35] mt-0.5 shrink-0" />
                    <div>
                      <p className="text-[11px] uppercase tracking-[0.15em] text-[#64748B]">Certifications</p>
                      <p className="text-[#0F172A] font-medium text-[13px] mt-0.5">{provider.certifications.join(', ')}</p>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="bg-white rounded-xl p-6 md:p-8 border border-[#E2E8F0]">
                ...
                <div className="p-5 bg-white border border-[#E2E8F0] rounded-lg">
                  <div className="flex items-baseline gap-2 flex-wrap">
                    ...
                    {provider.priceUnit && (
                      <span className="text-[#64748B] text-[13px]">/ {provider.priceUnit}</span>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-xl p-6 md:p-8 border border-[#E2E8F0]">
                ...
                <div className="space-y-4">
                  {provider.reviews.map((review) => (
                    <div key={review.id} className="p-4 bg-white rounded-lg border border-[#E2E8F0]">
                      <div className="flex items-center justify-between mb-2 gap-3">
                        <span className="font-semibold text-[#0F172A] text-[14px]">{review.authorName}</span>
                        <div className="flex items-center gap-0.5">
                          {Array.from({ length: 5 }).map((_, i) => (
                            <Star
                              key={i}
                              className={`w-3.5 h-3.5 ${i < review.rating ? 'text-[#FF6B35] fill-[#FF6B35]' : 'text-[#CBD5E1]'}`}
                            />
                          ))}
                        </div>
                      </div>
                      {review.title && <p className="font-medium text-[#0F172A] text-[14px] mb-1">{review.title}</p>}
                      <p className="text-[#334155] text-[13px] leading-relaxed">{review.comment}</p>
                      <p className="text-[#94A3B8] text-[11px] font-mono mt-2">
                        {new Date(review.createdAt).toLocaleDateString('fr-FR')}
                      </p>
                      {review.ownerResponse && (
                        <div className="mt-3 p-3 bg-[#F8FAFC] border-l-2 border-[#FF6B35] rounded-lg">
          ...
          <div className="space-y-6">
            {/* Contact Card */}
            <div className="bg-white rounded-xl p-6 border border-[#E2E8F0] lg:sticky lg:top-24">
              <p className="text-[11px] uppercase tracking-[0.18em] text-[#FF6B35] mb-2">Contacter</p>
              <h3 className="text-[18px] font-semibold tracking-[-0.02em] text-[#0F172A] mb-4">Réservation directe</h3>
              ...
              <div className="space-y-2.5">
                ...
                {provider.whatsapp && (
                  <a
                    ...
                    className="flex items-center gap-3 w-full px-4 py-3 bg-white border border-[#E2E8F0] hover:border-[#CBD5E1] text-[#0F172A] font-medium text-[14px] rounded-lg transition-colors"
                  >
            ...
            {similarProviders.length > 0 && (
              <div className="bg-white rounded-xl p-6 border border-[#E2E8F0]">
                ...
                <h3 className="text-[16px] font-semibold tracking-[-0.02em] text-[#0F172A] mb-4">Découvrir aussi</h3>
                <div className="space-y-2.5">
                  {similarProviders.map((sp) => (
                    <motion.div key={sp.id} whileHover={{ y: -2 }} transition={{ duration: 0.2 }}>
                      <Link
                        href={`/prestataires/${sp.slug}`}
                        className="flex items-center gap-3 p-3 bg-white border border-[#E2E8F0] hover:border-[#CBD5E1] rounded-lg transition-colors group"
                      >
                        <div className="w-11 h-11 rounded-lg overflow-hidden bg-[#F8FAFC] flex-shrink-0 relative border border-[#E2E8F0]">
                          <Image
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-medium text-[#0F172A] text-[13px] truncate group-hover:text-[#FF6B35] transition-colors">{sp.name}</p>
