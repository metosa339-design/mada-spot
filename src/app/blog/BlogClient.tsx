            ...
            <Link href={`/blog/${featured.slug}`} className="block group">
              <article className="relative rounded-xl overflow-hidden bg-white border border-[#E2E8F0] hover:border-[#CBD5E1] transition-colors">
                <div className="md:flex">
                  <div className="md:w-1/2 relative aspect-[16/10] md:aspect-auto bg-white">
                    {featured.imageUrl ? (
                      <Image
                      ...
                      <div className="w-full h-full min-h-[250px] bg-white flex items-center justify-center">
                  ...
                  <div className="md:w-1/2 p-6 md:p-8 flex flex-col justify-center">
                    <div className="flex items-center gap-2 mb-4 flex-wrap">
                      ...
                      {featured.category && (
                        <span className="px-2.5 py-1 rounded-md text-[10px] font-semibold uppercase tracking-[0.1em] bg-white border border-[#E2E8F0] text-[#334155]">
                          {featured.category.name}
                        </span>
                      )}
                    </div>
                    <h2 className="text-[20px] md:text-[26px] font-semibold tracking-[-0.02em] text-[#0F172A] mb-3 group-hover:text-[#FF6B35] transition-colors leading-tight">
                      {featured.title}
                    </h2>
                    <p className="text-[14px] text-[#64748B] mb-4 line-clamp-3 leading-relaxed">
                ...
                <Link href={`/blog/${article.slug}`} className="group block h-full">
                  <article className="h-full rounded-xl overflow-hidden bg-white border border-[#E2E8F0] hover:border-[#CBD5E1] transition-colors">
                    <div className="relative aspect-[16/10] bg-white">
                      ...
                      {article.category && (
                        <span className="absolute top-3 left-3 px-2.5 py-1 rounded-md text-[10px] font-semibold uppercase tracking-[0.1em] backdrop-blur-md bg-[#FFF7ED] border border-[#FF6B35]/30 text-[#FF6B35]">
                          {article.category.name}
                        </span>
                      )}
                    </div>
                    <div className="p-5">
                      <h3 className="font-semibold text-[#0F172A] text-[15px] mb-2 group-hover:text-[#FF6B35] transition-colors line-clamp-2 leading-snug">
                        {article.title}
                      </h3>
