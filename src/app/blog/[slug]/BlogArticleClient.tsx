    ...
    <>
      ...
      <div className="max-w-4xl mx-auto px-4 pt-28">
        <nav className="flex items-center gap-2 text-[12px] text-[#94A3B8]">
          <Link href="/" className="hover:text-[#0F172A] transition-colors">{t.breadcrumbHome}</Link>
          <span>/</span>
          <Link href="/blog" className="hover:text-[#0F172A] transition-colors">{t.breadcrumbBlog}</Link>
          <span>/</span>
          <span className="text-[#64748B] truncate">{article.title}</span>
        </nav>
      </div>
      ...
      <article className="max-w-4xl mx-auto px-4 pt-8 pb-20">
        <header className="mb-8">
          <div className="flex items-center gap-2 mb-4 flex-wrap">
            {article.category && (
              <span className="px-2.5 py-1 rounded-md text-[10px] font-semibold uppercase tracking-[0.1em] bg-[#FFF7ED] border border-[#FF6B35]/30 text-[#FF6B35]">
          ...
          <h1 className="text-[28px] md:text-[40px] font-semibold tracking-[-0.03em] text-[#0F172A] mb-4 leading-[1.1]">
            {article.title}
          </h1>

          {article.summary && (
            <p className="text-[16px] sm:text-[18px] text-[#334155] mb-6 leading-relaxed max-w-[65ch]">{article.summary}</p>
          )}

          <div className="flex items-center gap-4 text-[12px] font-mono text-[#94A3B8] border-b border-[#E2E8F0] pb-5">
        ...
        {/* Cover Image */}
        {article.imageUrl && (
          <div className="relative aspect-[16/9] rounded-xl overflow-hidden mb-10 border border-[#E2E8F0]">
            <Image
        ...
        <div
          className="prose prose-invert prose-lg max-w-[65ch]
            prose-headings:text-[#0F172A] prose-headings:font-semibold prose-headings:tracking-[-0.02em]
            ...
            prose-blockquote:border-[#FF6B35] prose-blockquote:text-[#64748B]
        ...
        {additionalImages.length > 0 && (
          <div className="mt-10 grid grid-cols-1 md:grid-cols-2 gap-4">
            {additionalImages.map((img: string, i: number) => (
              <div key={i} className="relative aspect-[16/10] rounded-xl overflow-hidden border border-[#E2E8F0]">
                <Image
                  src={img}
        ...
        {/* CTA */}
        <div className="mt-12 p-6 md:p-8 rounded-xl bg-white border border-[#E2E8F0] text-center">
          <h3 className="text-[20px] sm:text-[24px] font-semibold tracking-[-0.02em] text-[#0F172A] mb-2">{t.ctaTitle}</h3>
          <p className="text-[#64748B] text-[14px] mb-5 leading-relaxed">{t.ctaDesc}</p>
          <Link
        ...
        {related.length > 0 && (
          <section className="mt-16">
            ...
            <h2 className="text-[22px] sm:text-[28px] font-semibold tracking-[-0.02em] text-[#0F172A] mb-6">{t.relatedArticles}</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {related.map((r: any) => (
                <motion.div key={r.id} whileHover={{ y: -2 }} transition={{ duration: 0.2 }}>
                  <Link href={`/blog/${r.slug}`} className="group block h-full">
                    <article className="rounded-xl overflow-hidden bg-white border border-[#E2E8F0] hover:border-[#CBD5E1] transition-colors h-full">
                      ...
                      <div className="p-4">
                        <h3 className="font-semibold text-[#0F172A] text-[13px] group-hover:text-[#FF6B35] transition-colors line-clamp-2 leading-snug">
                          {r.title}
                        </h3>
                        <p className="text-[11px] font-mono text-[#94A3B8] mt-2">
