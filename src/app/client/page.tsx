    ...
    >
      ...
      <div className="flex gap-2 mb-2">
        <input
          ...
          placeholder="Montant"
          className="flex-1 bg-black/30 border border-emerald-500/20 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-emerald-400/50 w-0 min-w-0"
        />
        <select
          value={currency}
          onChange={e => setCurrency(e.target.value)}
          className="bg-black/30 border border-emerald-500/20 rounded-lg px-2 py-2 text-sm text-white focus:outline-none"
        >
          {Object.entries(EXCHANGE_RATES).map(([key, v]) => (
            <option key={key} value={key} className="bg-[#1a1a24]">{v.symbol} {key}</option>
          ))}
        </select>
      </div>
      {converted > 0 && (
    ...
    <div className="min-h-screen bg-[#0a0a0f] text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-12">
        ...
        <motion.div variants={slideUp} custom={1} initial="hidden" animate="visible" className="mb-8">
          {nextBooking ? (
            <div className="relative overflow-hidden rounded-2xl border border-[#2a2a36]">
              {/* Background */}
                ...
                <div className="flex flex-col gap-2 shrink-0">
                  {nextBooking.establishment.latitude && nextBooking.establishment.longitude && (
                    <a
                      ...
                      className="flex items-center gap-2 px-5 py-3 bg-[#0891b2] text-white rounded-xl text-sm font-semibold hover:bg-[#0891b2]/90 transition-colors shadow-lg shadow-cyan-500/20"
                    >
                  ...
                  <Link
                    ...
                    className="flex items-center gap-2 px-5 py-2.5 bg-white/5 border border-white/10 text-gray-300 rounded-xl text-sm hover:bg-white/10 transition-colors text-center justify-center"
                  >
            ...
            <div className="relative overflow-hidden rounded-2xl border border-[#2a2a36] bg-gradient-to-br from-[#ff6b35]/10 via-[#0a0a0f] to-pink-500/5">
                ...
                <form onSubmit={handleSearch} className="flex gap-2 max-w-lg">
                  <div className="flex-1 relative">
                    ...
                    <input
                      ...
                      className="w-full pl-10 pr-4 py-3 bg-[#1a1a24] border border-[#2a2a36] rounded-xl text-sm text-white placeholder-gray-500 focus:outline-none focus:border-[#ff6b35]/50 transition-colors"
                    />
                  </div>
                  <button
                    type="submit"
            ...
            <motion.div key={stat.label} variants={slideUp} custom={index + 2} initial="hidden" animate="visible">
              <Link href={stat.href}>
                <motion.div
                  ...
                  className="bg-[#1a1a24] rounded-2xl p-5 border border-[#2a2a36] hover:border-[#ff6b35]/20 transition-all cursor-pointer h-full"
                >
                  <div className="flex items-center justify-between mb-3">
                    <div
                  ...
                  {stat.value > 0 ? (
                    <>
                      <p className="text-2xl font-bold text-white">{stat.value.toLocaleString('fr-FR')}</p>
                      {stat.subtitle && (
        ...
        <motion.div variants={slideUp} custom={6} initial="hidden" animate="visible" className="mb-8">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-white">Explorer par catégorie</h2>
              ...
              return (
                <Link key={item.label} href={item.href}>
                  <motion.div
                    ...
                    className={`relative bg-[#1a1a24] border border-[#2a2a36] rounded-2xl p-6 hover:border-[#ff6b35]/20 transition-all cursor-pointer overflow-hidden group`}
                  >
                    ...
                    <div className="relative">
                      ...
                      <h3 className="font-semibold text-white mb-1 group-hover:text-[#ff6b35] transition-colors">{item.label}</h3>
        ...
        {recentFavorites.length > 0 && (
          <motion.div variants={slideUp} custom={7} initial="hidden" animate="visible" className="mb-8">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-white">Vos derniers favoris</h2>
              ...
              {recentFavorites.map((fav) => {
                ...
                return (
                  <Link key={fav.id} href={href} className="shrink-0 w-44">
                    <motion.div
                      whileHover={{ y: -4 }}
                      className="bg-[#1a1a24] border border-[#2a2a36] rounded-xl overflow-hidden hover:border-[#ff6b35]/20 transition-all"
                    >
                      <div className="relative h-24 bg-gradient-to-br from-[#ff6b35]/10 to-pink-500/10">
                        ...
                        {est.rating && est.rating > 0 && (
                          <div className="absolute bottom-1.5 left-1.5 flex items-center gap-0.5 bg-black/60 backdrop-blur-sm rounded px-1.5 py-0.5">
                            ...
                            <span className="text-white text-[10px] font-medium">{est.rating.toFixed(1)}</span>
                          </div>
                        )}
                      </div>
                      <div className="p-3">
                        <h4 className="text-sm font-medium text-white truncate">{est.name}</h4>
            ...
            {[
              ...
              <Link key={item.label} href={item.href}>
                <motion.div
                  ...
                  whileTap={{ scale: 0.97 }}
                  className="relative bg-[#1a1a24] border border-[#2a2a36] rounded-2xl p-4 flex flex-col items-center gap-2 hover:border-[#ff6b35]/20 transition-all cursor-pointer text-center"
                >
