      ...
      {data.map((val, i) => (
        <div key={i} className="flex-1 flex flex-col items-center gap-1">
          <div
            ...
            title={`${labels[i]}: ${val}`}
          />
          <span className="text-[9px] text-[#94A3B8] leading-none">{labels[i]}</span>
        </div>
      ))}
    </div>
  )
      ...
      <div className="space-y-2">
        {checks.map((check, i) => (
          <div key={i} className="flex items-center gap-2 text-xs">
            ...
            <span className={check.done ? 'text-[#94A3B8]' : 'text-[#94A3B8]'}>{check.label}</span>
          </div>
        ))}
      </div>
    </div>
...
function StatCard({ label, value, icon: Icon, trend, color, href, sparkData }: {
  ...
  const content = (
    <motion.div
      ...
      className="bg-white border border-[#E2E8F0] rounded-2xl p-5 cursor-pointer transition-shadow hover:shadow-lg relative overflow-hidden"
  ...
  return (
    <div className="relative w-24 h-24 mx-auto">
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <span className="text-xl font-bold text-[#0F172A]">{Math.round(rate)}%</span>
      </div>
    </div>
  )
}
...
function ResponseTimeBadge({ hours, t }: { hours: number; t: DashboardProTrans }) {
  ...
  return (
    <div className={`flex items-center gap-3 p-4 rounded-xl border border-[#E2E8F0] ${config.bg}`}>
      <div className={`relative ${isPoor ? 'animate-pulse' : ''}`}>
        <Clock className="w-8 h-8" style={{ color: config.color }} />
      </div>
      <div className="flex-1">
        <p className="text-xs text-[#94A3B8]">{t.avgResponseTime}</p>
        <p className="text-lg font-bold text-[#0F172A]">{hours > 0 ? `${hours}h` : '—'}</p>
      </div>
...
function BookingRow({ booking, accent, t }: { booking: BookingItem; accent: string; t: DashboardProTrans }) {
  ...
  return (
    <div className="flex items-center gap-4 p-4 hover:bg-white rounded-xl transition-colors">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-[#0F172A] truncate">{booking.guestName}</p>
      ...
      <div className="text-right">
        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${statusColors[booking.status] || 'bg-[#F1F5F9] text-[#94A3B8]'}`}>
          {statusLabels[booking.status] || booking.status}
        </span>
        {booking.totalPrice != null && (
          <p className="text-xs text-[#94A3B8] mt-1">{booking.totalPrice.toLocaleString('fr-FR')} MGA</p>
        )}
      </div>
    </div>
  )
  ...
  return (
    <div className="flex items-center gap-4 p-4 hover:bg-white rounded-xl transition-colors">
      ...
      <div className="flex items-center gap-2">
        {booking.totalPrice != null && (
          <span className="text-xs text-[#94A3B8] mr-2">{booking.totalPrice.toLocaleString('fr-FR')} Ar</span>
        )}
        <button
...
export default function DashboardHome() {
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[...Array(8)].map((_, i) => (
            <div key={i} className="bg-white rounded-2xl p-5 animate-pulse">
              <div className="w-11 h-11 bg-white rounded-xl mb-4" />
              <div className="h-7 w-20 bg-white rounded mb-2" />
      ...
      <div className="space-y-6 max-w-7xl">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-[#94A3B8] mt-1">{t.hotelDashboardSubtitle}</p>
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="bg-white border border-[#E2E8F0] rounded-2xl p-5">
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-sm text-[#94A3B8]">{t.occupancyRate}</p>
            </div>
            <OccupancyGauge rate={stats.occupancyRate || 0} color="#0891b2" />
            <p className="text-center text-xs text-[#94A3B8] mt-2">{t.thisMonth}</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-white border border-[#E2E8F0] rounded-2xl overflow-hidden">
            <div className="flex items-center justify-between p-5 border-b border-[#E2E8F0]">
              ...
              <span className="text-xs text-[#94A3B8]">{todayArrivals.length} {todayArrivals.length !== 1 ? t.checkInPlural : t.checkInSingle}</span>
            </div>
            <div className="divide-y divide-[#E2E8F0]">
              {todayArrivals.length > 0 ? todayArrivals.map((arrival, i) => (
                <div key={i} className="flex items-center gap-4 p-4 hover:bg-white transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-[#0F172A] truncate">{arrival.guestName}</p>
                    <p className="text-xs text-[#94A3B8]">
                ...
                <div className="p-8 text-center">
                  <CalendarDays className="w-10 h-10 text-[#64748B] mx-auto mb-3" />
                  <p className="text-sm text-[#94A3B8]">{t.noArrivalToday}</p>
                </div>
              )}
            </div>
          </div>
        ...
        {pendingList.length > 0 && (
          <AnimatePresence>
            ...
            >
              <div className="flex items-center justify-between p-5 border-b border-[#E2E8F0] bg-amber-500/5">
                <h3 className="text-sm font-semibold text-[#0F172A] flex items-center gap-2">
            ...
            <div className="space-y-4">
              <div>
                <div className="flex justify-between text-xs mb-1">
                  <span className="text-[#94A3B8]">{t.confirmationRate}</span>
              ...
              <div>
                <div className="flex justify-between text-xs mb-1">
                  <span className="text-[#94A3B8]">{t.answeredReviews}</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-white border border-[#E2E8F0] rounded-2xl overflow-hidden">
            <div className="flex items-center justify-between p-5 border-b border-[#E2E8F0]">
              <h2 className="text-lg font-semibold text-[#0F172A]">{t.recentBookings}</h2>
            ...
            <div className="divide-y divide-[#E2E8F0]">
              {recentBookings.length > 0 ? (
                ...
                <div className="p-8 text-center">
                  ...
                  <p className="text-sm text-[#94A3B8]">{t.noRecentBookings}</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-white border border-[#E2E8F0] rounded-2xl overflow-hidden">
            <div className="flex items-center justify-between p-5 border-b border-[#E2E8F0]">
              <h2 className="text-lg font-semibold text-[#0F172A]">{t.recentReviews}</h2>
            ...
            <div className="divide-y divide-[#E2E8F0]">
              {recentReviews.length > 0 ? (
                recentReviews.slice(0, 5).map((review) => (
                  <div key={review.id} className="p-4 hover:bg-white transition-colors">
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-[#0F172A] truncate">{review.authorName || t.anonymous}</p>
                        <div className="flex items-center gap-1 mt-0.5">
                          {[...Array(5)].map((_, i) => (
                            <Star key={i} className={`w-3 h-3 ${i < review.rating ? 'text-yellow-400 fill-yellow-400' : 'text-[#64748B]'}`} />
                          ))}
                        </div>
                      </div>
                ...
                <div className="p-8 text-center">
                  <Star className="w-10 h-10 text-[#64748B] mx-auto mb-3" />
                  <p className="text-sm text-[#94A3B8]">{t.noRecentReviews}</p>
                </div>
              )}
            </div>
          </div>
  ...
  return (
    <div className="space-y-6 max-w-7xl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-[#0F172A]">{greeting()}, {user?.firstName} !</h2>
          <p className="text-[#94A3B8] mt-1">{t.genericDashboardSubtitle}</p>
        </div>
        <div className="flex flex-wrap gap-2">
          {pendingBookings > 0 && (
            <Link href="/dashboard/reservations?tab=pending" className="flex items-center gap-2 px-4 py-2.5 bg-[#FF6B35]/15 border border-[#FF6B35]/30 rounded-xl text-[#FDBA74] text-sm font-semibold hover:bg-[#FFEDD5] transition-colors animate-pulse">
              <AlertTriangle className="w-4 h-4" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white border border-[#E2E8F0] rounded-2xl p-5">
          ...
          <div className="space-y-4">
            <div>
              <div className="flex justify-between text-xs mb-1">
                <span className="text-[#94A3B8]">{t.avgResponseTime}</span>
        ...
        {[
          ...
          <Link key={action.href} href={action.href}>
            <motion.div
              whileHover={{ y: -2 }} transition={{ duration: 0.2 }}
              whileTap={{ scale: 0.97 }}
              className="bg-white border border-[#E2E8F0] rounded-2xl p-4 flex flex-col items-center gap-2 hover:border-[#FF6B35]/30 transition-colors cursor-pointer text-center"
            >
