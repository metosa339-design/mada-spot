    ...
    <div className="flex items-end gap-2 h-32">
      {data.map((item, i) => (
        <div key={i} className="flex-1 flex flex-col items-center gap-1">
          ...
          />
          <span className="text-[10px] text-[#94A3B8] truncate max-w-full">{item.label}</span>
        </div>
      ))}
    </div>
  )
  ...
  return (
    <div className="space-y-8 max-w-6xl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-[#0F172A]">{t.title}</h1>
          <p className="text-[#94A3B8] text-sm mt-1">{t.pageSubtitle}</p>
        </div>
        <div className="flex gap-1 bg-white border border-[#E2E8F0] rounded-xl p-1 self-start sm:self-auto">
          {[
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                period === p.value ? 'bg-[#FF6B35] text-white' : 'text-[#94A3B8] hover:text-white'
              }`}
            >
              {p.label}
            </button>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {statCards.map((card, i) => (
          ...
          >
            ...
            <p className="text-xs text-[#94A3B8] mt-1 mb-3">{card.label}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white border border-[#E2E8F0] rounded-2xl p-6">
          ...
          {stats?.monthlyRevenue && stats.monthlyRevenue.length > 0 ? (
            ...
            <div className="h-32 flex items-center justify-center text-[#94A3B8] text-sm">
              {t.noData}
            </div>
          )}
        </div>
      ...
      {analytics?.funnel && (
        <div className="bg-white border border-[#E2E8F0] rounded-2xl p-6">
          <h3 className="text-sm font-medium text-[#0F172A] mb-5">{t.conversionFunnel}</h3>
          <div className="space-y-3">
            {[
                ...
                <div key={step.label}>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {convRate !== null && (
                        <span className="text-[10px] text-[#94A3B8]">({convRate}%)</span>
                      )}
                    </div>
                  </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {analytics?.occupancyByDay && (
          <div className="bg-white border border-[#E2E8F0] rounded-2xl p-6">
            <h3 className="text-sm font-medium text-[#0F172A] mb-5">{t.occupancyByDay}</h3>
            <div className="grid grid-cols-7 gap-2">
              {analytics.occupancyByDay.map((d, i) => (
                ...
                >
                  <div
                    className="w-full aspect-square rounded-xl flex items-center justify-center border border-[#E2E8F0]"
                    style={{
        ...
        {analytics?.revenueComparison && analytics.revenueComparison.current.length > 0 && (
          <div className="bg-white border border-[#E2E8F0] rounded-2xl p-6">
            <h3 className="text-sm font-medium text-[#0F172A] mb-2">{t.currentVsPrevious}</h3>
            <div className="flex items-center gap-4 mb-4">
              <div className="flex items-center gap-1.5">
                <div className="w-3 h-3 rounded-sm bg-[#FF6B35]" />
                <span className="text-[10px] text-[#94A3B8]">{t.currentLegend}</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className="w-3 h-3 rounded-sm bg-white" />
                <span className="text-[10px] text-[#94A3B8]">{t.previousLegend}</span>
              </div>
            </div>
                ...
                return (
                  <div key={week.label} className="flex-1 flex items-end gap-0.5">
                    <motion.div
                      ...
                      transition={{ delay: i * 0.05, duration: 0.5 }}
                      className="flex-1 rounded-t bg-white min-h-[2px]"
                    />
                    <motion.div
                      initial={{ height: 0 }}
            ...
            <div className="flex gap-1 mt-1">
              {analytics.revenueComparison.current.map(w => (
                <span key={w.label} className="flex-1 text-center text-[9px] text-[#94A3B8]">{w.label}</span>
              ))}
            </div>
          </div>
        )}
      ...
      <div className="bg-white border border-[#E2E8F0] rounded-2xl p-5">
        <h3 className="text-sm font-medium text-[#0F172A] mb-2">{t.seoTitle}</h3>
        ...
        <div className="mt-3 flex items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-[#94A3B8]">{t.structuredDataActive}</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 rounded-full bg-emerald-400" />
            <span className="text-xs text-[#94A3B8]">{t.schemaLocalBusiness}</span>
          </div>
        </div>
      </div>
    </div>
