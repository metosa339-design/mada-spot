...
export default function PhotoGallerySection({ images, gallery, coverImage }: PhotoGallerySectionProps) {
  ...
  return (
    <>
      <section className="bg-white rounded-2xl p-4 sm:p-6 md:p-8 border border-[#2a2a36]">
        <div className="flex items-center gap-3 mb-4 sm:mb-6">
          <Camera className="w-5 h-5 text-[#FDBA74]" />
          <h2 className="text-lg sm:text-xl font-bold text-white">Galerie Photos</h2>
          <span className="text-xs sm:text-sm text-[#94A3B8]">({allItems.length})</span>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 sm:gap-3">
          {allItems.map((item, index) => (
            <motion.div
              ...
              whileTap={{ scale: 0.98 }}
              className="group relative aspect-[4/3] rounded-lg sm:rounded-xl overflow-hidden cursor-pointer bg-[#F8FAFC]"
      ...
      <AnimatePresence>
        {lightboxIndex !== null && (
          ...
          >
            ...
            <button
              onClick={closeLightbox}
              className="absolute top-3 right-3 sm:top-4 sm:right-4 p-2 text-white/70 hover:text-white bg-white rounded-full z-10"
            >
            ...
            <button
              ...
              className="hidden sm:block absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 p-2 sm:p-3 bg-white rounded-full text-white hover:bg-white z-10"
            >
              <ChevronLeft className="w-5 h-5 sm:w-6 sm:h-6" />
            </button>
            <button
              onClick={(e) => { e.stopPropagation(); nextImage(); }}
              className="hidden sm:block absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 p-2 sm:p-3 bg-white rounded-full text-white hover:bg-white z-10"
            >
            ...
            <div className="sm:hidden absolute bottom-28 left-0 right-0 flex justify-between px-4 z-10">
              <button
                onClick={(e) => { e.stopPropagation(); prevImage(); }}
                className="p-3 bg-white rounded-full text-white active:bg-[#F8FAFC]"
              >
