...
export default function Header() {
  ...
  return (
    <>
      ...
      <header
        role="banner"
        className="relative z-50 bg-[#0A0A0F] border-b border-[#27272A]"
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 md:h-[68px]">
            ...
            <Link href="/" className="flex items-center gap-2.5 md:gap-3 shrink-0">
              ...
              />
              <span
                className="text-[18px] md:text-[20px] font-semibold tracking-[-0.02em] text-[#FAFAFA]"
            ...
            >
                  ...
                  <AnimatePresence>
                    {isBonsPlansOpen && (
                      <motion.div
                        ...
                        className="absolute top-full left-0 mt-2 w-64 bg-[#111114] rounded-xl border border-[#27272A] overflow-hidden shadow-[0_24px_60px_-12px_rgba(0,0,0,0.6)]"
                      >
                        <div className="p-1.5">
                          {bonsPlansItems.map((item) => (
                            <Link
                              ...
                              className="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-[#1A1A1F] transition-colors group"
                            >
                              <div className="w-8 h-8 rounded-md bg-[#1A1A1F] border border-[#27272A] flex items-center justify-center group-hover:border-[#3F3F46] transition-colors">
                                <item.icon className="w-4 h-4 text-[#A1A1AA] group-hover:text-[#FAFAFA] transition-colors" />
                              </div>
                              <span className="text-[14px] font-medium text-[#D4D4D8] group-hover:text-[#FAFAFA] transition-colors">
                                {item.name}
                              </span>
                            </Link>
                          ))}
              ...
              <Link
                href="/blog"
                className={`flex items-center gap-1.5 px-3 py-2 rounded-md text-[14px] font-medium transition-colors ${
                  isActive('/blog') ? 'text-[#FAFAFA]' : 'text-[#D4D4D8] hover:text-[#FAFAFA]'
                }`}
              >
            ...
            <div className="flex items-center gap-1.5 md:gap-2">
              ...
              <button
                ...
                className="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-md bg-[#111114] border border-[#27272A] text-[#A1A1AA] hover:text-[#FAFAFA] hover:border-[#3F3F46] transition-colors"
              >
                ...
                <>
                  {user ? (
                    <div className="flex items-center gap-2">
                      ...
                      <div className="relative">
                        <button
                          ...
                          className="flex items-center gap-2 px-2 py-1.5 rounded-md text-[#FAFAFA] hover:bg-[#1A1A1F] transition-colors"
                        >
                          <div
                          ...
                          <ChevronDown
                            className={`w-3.5 h-3.5 transition-transform text-[#A1A1AA] ${isUserMenuOpen ? 'rotate-180' : ''}`}
                            aria-hidden="true"
                          />
                        </button>
                        ...
                        <AnimatePresence>
                          {isUserMenuOpen && (
                            <motion.div
                              ...
                              exit={{ opacity: 0, y: 8 }}
                              transition={{ duration: 0.18, ease: EASE }}
                              className="absolute top-full right-0 mt-2 w-60 bg-[#111114] rounded-xl border border-[#27272A] overflow-hidden shadow-[0_24px_60px_-12px_rgba(0,0,0,0.6)]"
                            >
                              ...
                              <div className="py-1.5">
                                {user.role === 'ADMIN' ? (
                                  <Link
                                    ...
                                    className="flex items-center gap-3 px-4 py-2 text-[13px] text-[#D4D4D8] hover:bg-[#1A1A1F] hover:text-[#FAFAFA] transition-colors"
                                  >
                    ...
                    <div className="hidden md:flex items-center gap-2">
                      <Link
                        href="/login"
                        className="px-3 py-1.5 rounded-md text-[13px] font-medium text-[#D4D4D8] hover:text-[#FAFAFA] transition-colors"
                      >
                        {t.login}
                      </Link>
                      <Link
              ...
              <button
                ...
                className="lg:hidden p-2 rounded-md text-[#FAFAFA] hover:bg-[#1A1A1F] transition-colors min-w-[40px] min-h-[40px] flex items-center justify-center"
              >
                {isMobileMenuOpen ? (
        ...
        <AnimatePresence>
          {isMobileMenuOpen && (
            <motion.div
              ...
              className="lg:hidden bg-[#0A0A0F] border-t border-[#27272A] overflow-hidden"
              role="navigation"
              aria-label={t.mobileMenu}
            >
              <div className="px-4 py-4 space-y-1">
                {/* Recherche mobile */}
                <div className="flex items-center gap-2 px-3 py-2.5 bg-[#111114] border border-[#27272A] rounded-lg mb-3">
                  <Search className="w-4 h-4 text-[#71717A]" aria-hidden="true" />
                  <input
                    type="text"
                    placeholder={t.searchDestination}
                    aria-label={t.searchDestination}
                    className="flex-1 bg-transparent outline-none text-[14px] text-[#FAFAFA] placeholder:text-[#71717A]"
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') {
                ...
                {!user?.userType && (
                  <div className="pb-3 border-b border-[#27272A] space-y-0.5">
                    <p className="text-[11px] uppercase tracking-[0.08em] text-[#71717A] px-3 pt-1 pb-1.5">
                      {t.bonsPlans}
                    </p>
                    {bonsPlansItems.map((item) => (
                      <Link
                        ...
                        className="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-[#111114] transition-colors"
                      >
                        <item.icon className="w-4 h-4 text-[#A1A1AA]" />
                        <span className="text-[14px] font-medium text-[#D4D4D8]">{item.name}</span>
                      </Link>
                    ))}
                  </div>
                )}
                ...
                {!user?.userType && (
                  <Link
                    href="/evenements"
                    onClick={() => setIsMobileMenuOpen(false)}
                    className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-[14px] font-medium text-[#D4D4D8] hover:bg-[#111114] transition-colors"
                  >
                ...
                <Link
                  href="/comment-ca-marche"
                  onClick={() => setIsMobileMenuOpen(false)}
                  className="block px-3 py-2.5 rounded-lg text-[14px] font-medium text-[#D4D4D8] hover:bg-[#111114] transition-colors"
                >
                  {t.howItWorks}
                </Link>

                {!user && (
                  <div className="pt-3 mt-2 border-t border-[#27272A] space-y-2">
                    <Link
                      href="/login"
                      onClick={() => setIsMobileMenuOpen(false)}
                      className="block w-full px-4 py-2.5 text-center text-[13px] font-medium text-[#FAFAFA] bg-[#111114] border border-[#27272A] rounded-lg hover:border-[#3F3F46] transition-colors"
                    >
                      {t.login}
                    </Link>
                    <Link
