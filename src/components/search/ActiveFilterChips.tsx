...
const TYPE_COLORS: Record<string, string> = {
  ...
  ATTRACTION: 'bg-[#FFEDD5] text-[#FDBA74] border-[#FF6B35]/30',
...
export default function ActiveFilterChips({ filters, clearFilter }: ActiveFilterChipsProps) {
  ...
  if (filters.type) {
    ...
    chips.push({
      ...
      color: TYPE_COLORS[filters.type] || 'bg-[#F1F5F9] text-[#CBD5E1] border-[#E2E8F0]',
    });
  }
  ...
  if (filters.amenities) {
    ...
    if (amenityValues.length > 0) {
      ...
      chips.push({
        ...
        color: 'bg-[#FFEDD5] text-[#FDBA74] border-[#FF6B35]/30',
      });
    }
  }
