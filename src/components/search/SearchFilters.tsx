...
const COLOR_CLASSES: Record<string, { active: string; border: string; text: string }> = {
  ...
  orange: { active: 'bg-[#FF6B35]/20 border-[#FF6B35] text-[#FDBA74]', border: 'border-[#FF6B35]/30', text: 'text-[#FDBA74]' },
...
export default function SearchFilters({ filters, setFilter, clearFilters, activeFilterCount }: SearchFiltersProps) {
  ...
  return (
    <div className="space-y-6">
      {/* Type filter */}
      <div>
        <h3 className="text-sm font-semibold text-[#52525B] mb-3 uppercase tracking-wider">Type</h3>
      ...
      {/* City filter */}
      <div>
        <h3 className="text-sm font-semibold text-[#52525B] mb-3 uppercase tracking-wider">Ville</h3>
        <input
          type="text"
          value={filters.city}
          onChange={(e) => setFilter('city', e.target.value)}
          placeholder="Ex: Antananarivo..."
          className="w-full px-3 py-2.5 rounded-lg bg-[#1A1A1F] border border-[#27272A] text-[#FAFAFA] placeholder:text-[#71717A] text-sm focus:outline-none focus:border-[#FF6B35]/50 focus:ring-1 focus:ring-orange-500/25 transition-colors"
        />
      </div>

      {/* Region filter */}
      <div>
        <h3 className="text-sm font-semibold text-[#52525B] mb-3 uppercase tracking-wider">Région</h3>
        <input
          type="text"
      ...
      {/* Minimum rating */}
      <div>
        <h3 className="text-sm font-semibold text-[#52525B] mb-3 uppercase tracking-wider">Note minimum</h3>
        <div className="flex flex-wrap gap-2">
          {RATING_OPTIONS.map((r) => {
            ...
            return (
              ...
              >
                <svg className={`w-3.5 h-3.5 ${isActive ? 'text-yellow-400 fill-yellow-400' : 'text-[#71717A]'}`} viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" strokeWidth="1">
      ...
      {/* Price range */}
      <div>
        <h3 className="text-sm font-semibold text-[#52525B] mb-3 uppercase tracking-wider">Gamme de prix</h3>
        <div className="grid grid-cols-2 gap-2">
      ...
      {/* Amenities */}
      <div>
        <h3 className="text-sm font-semibold text-[#52525B] mb-3 uppercase tracking-wider">Équipements</h3>
            ...
            return (
              <label
                ...
                className={`flex items-center gap-3 px-3 py-2 rounded-lg cursor-pointer transition-colors ${
                  isChecked ? 'bg-[#FF6B35]/10 text-[#FDBA74]' : 'text-[#71717A] hover:bg-[#1A1A1F] hover:text-[#52525B]'
                }`}
              >
                <input
                  type="checkbox"
                ...
                <div className={`w-4 h-4 rounded border flex items-center justify-center transition-colors ${
                  isChecked ? 'bg-[#FF6B35] border-[#FF6B35]' : 'border-[#3F3F46]'
                }`}>
                  {isChecked && (
